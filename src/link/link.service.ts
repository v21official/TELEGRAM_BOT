...
export class LinkService {
    ...
    async create(req: any, createLinkDto: CreateLinkDto): Promise<any> {
        if (!createLinkDto.original)
            throw new BadRequestException('Vui long nhap link can rut gon');
        let alias = '';
        if (createLinkDto.alias) {
            ...
            if (checkExistAlias)
                throw new BadRequestException('Phan rut gon da ton tai. Vui long nhap lai');
        } else {
    ...
    async findAllByUsername(username: string): Promise<Link[]> {
        const data = await this.linkModel.find({ username, isActive: false }).exec();
        return data;
    }

    async findLinkByAlias(alias: string): Promise<Link> {
        const link = await this.linkModel.findOne({ alias, isActive: false });
        return link;
    }
    ...
    async delete(req: any, alias: string): Promise<any> {
        const findLink = await this.linkModel.findOne({
            alias,
            username: req.user.username,
            isActive: false,
        });
        if (!findLink) throw new NotFoundException('Khong tim thay link can xoa');
        const linkDeleted = await this.linkModel.findByIdAndUpdate(
            { _id: findLink.id },
            { isActive: true },
